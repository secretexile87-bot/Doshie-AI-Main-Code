...
sys.path.insert(0, "/data/data/com.termux/files/home/yoshi")
import yoshi_memory
import yoshi_agents
import yoshi_news
import yoshi_profile_avatar
import yoshi_profile_lock
import yoshi_profile_preferences
import yoshi_search
import yoshi_settings
import yoshi_history
import yoshi_health_access
import yoshi_invites
import yoshi_recovery
import yoshi_roles
import yoshi_summary
import yoshi_voice_proxy
import yoshi_spotify
import yoshi_messaging
import yoshi_permissions
import yoshi_equipment_health
import yoshi_chat_attachments
import yoshi_media
...
app.secret_key = yoshi_profile_lock.get_session_secret()
app.config.update(
    MAX_CONTENT_LENGTH=10 * 1024 * 1024,
    SESSION_COOKIE_NAME="diyoshi_session",
...
<meta name="apple-mobile-web-app-title" content="DiYoshi">
<link rel="manifest" href="/static/manifest.webmanifest">
<link rel="icon" href="/static/yoshi-icon.svg" type="image/svg+xml">
<link rel="apple-touch-icon" href="/static/yoshi-192.png">
<link rel="stylesheet" href="/static/yoshi-app.css?v=56">
<title>DiYoshi</title>
...
.yoshi { background:var(--surface-soft); border:1px solid var(--line); border-bottom-left-radius:5px; }
...
<section id="accountChooser" class="account-chooser" hidden
    ...
    <div class="account-chooser-card">
        ...
        <h1 id="accountChooserTitle">Who’s using DiYoshi?</h1>
        ...
        <form id="publicSignInPanel" class="invite-access-panel"
            ...
            <p>Enter your DiYoshi username and private password or PIN.</p>
            <input id="publicSignInUsername" type="text"
                   name="diyoshi-login-name" autocomplete="new-password" autocapitalize="none"
...
<aside class="sidebar" aria-label="DiYoshi navigation">
    <div class="sidebar-brand">
        <span class="brand-mark">🦖</span><span class="side-text">DiYoshi</span>
    </div>
    ...
    <nav class="side-actions">
        ...
        <button class="side-action" data-side-view="mansion"
            ...
            <span class="side-icon">🏰</span><span class="side-text">Mansion DiYoshi</span>
        </button>
...
<div class="app">
    ...
    <div class="header">
        ...
        <button class="install-app"
                data-install-app
                hidden
                onclick="installYoshiApp()">Install</button>
        ...
        <small id="connectionStatus">🟡 DiYoshi is waking up...</small>
    </div>
    ...
    <div id="gamingPanel" style="
        ...
        border-bottom:1px solid #2b3932;
    ">
        <h3 style="margin-top:0;">🎮 DiYoshi Gaming</h3>
    ...
    ">
        <div class="panel-heading">
            <strong>📘 DiYoshi Mini AI Tutorial</strong>
    ...
    <div id="techPanel" style="
        ...
        border-bottom:1px solid #2b3932;
    ">
        <h3 style="margin-top:0;">💻 DiYoshi Tech</h3>
    ...
    function renderHermesHistory(messages) {
        ...
        if (!messages.length) {
            ...
            empty.textContent = "Hermes is ready. Ask for help with DiYoshi, coding, equipment, or planning.";
            list.append(empty);
            return;
        }
    ...
    async function sendHermesMessage(event) {
        ...
        try {
            ...
            list.scrollTop = list.scrollHeight;
            status.textContent = "";
            if (yoshiSettings.speak_replies && typeof speakYoshiReply === "function") {
                speakYoshiReply(data.reply);
            }
        } catch (error) {
            status.textContent = error.message;
        }
    ...
    function diyoshiNativeSpeechPlugin() {
        ...
        return typeof capacitor.registerPlugin === "function"
            ? capacitor.registerPlugin("DiYoshiSpeech")
            : null;
    }

    async function startHermesVoice() {
        const status = document.getElementById("hermesStatus");
        const nativeSpeech = diyoshiNativeSpeechPlugin();
        if (nativeSpeech) {
    ...
    <div id="builderPanel" class="builder-panel" style="display:none;">
        <div class="panel-heading">
            <strong>🧩 Build with DiYoshi</strong>
            <button type="button" class="panel-close" onclick="closeBuilderPanel()">✕</button>
        </div>
        <p class="builder-subtitle">Describe an app or code change. DiYoshi will plan it, show the files it wants to change, test the result, and wait for your approval.</p>
        <label class="settings-field" for="builderGoal">
            <span>What should DiYoshi build?</span>
        ...
        <small class="permission-note">Approval lock: DiYoshi proposes changes first. Nothing is written, installed, or restarted without your approval.</small>
    ...
    function openBuilderPanel() {
        ...
        if (!record || !record.is_admin) {
            statusBox.textContent = "Administrator access is required for Build with DiYoshi.";
            return;
        }
    ...
    <div id="adminPanel" style="display:none;">
        <div class="panel-heading">
            <strong>🛡️ DiYoshi Admin Control Center</strong>
            <button type="button" class="panel-close" onclick="closeAdminControl()">✕</button>
        </div>
        <div class="admin-control-grid">
            <section class="admin-card admin-card-wide">
                <small>LIVE STATUS</small>
                <h2>DiYoshi command center</h2>
                <p id="adminControlSummary">Checking DiYoshi...</p>
            ...
            <section class="admin-card admin-card-wide">
                ...
                <strong id="equipmentHealthSummary">Checking this DiYoshi host...</strong>
            ...
            <section class="admin-card admin-card-wide">
                ...
                <div class="admin-actions">
                    <button class="tool" onclick="talkToYoshiFromControl()">💬 Talk to DiYoshi</button>
                    ...
                    <button class="tool" onclick="restartYoshiService()">Restart DiYoshi</button>
                </div>
            </section>
            <section class="admin-card admin-card-wide terminal-card">
                <div class="terminal-heading">
                    <div>
                        ...
                        <p>Customize DiYoshi colors, interface code, model behavior, backups, and services as your normal Linux user.</p>
                    </div>
                    <span class="terminal-user-badge">USER ONLY</span>
                </div>
                <div class="terminal-hints">
                    <button type="button" class="tool" onclick="setAdminTerminalCommand('cd /home/hermes-duran/yoshi && code static/yoshi-app.css')">Open theme CSS</button>
                    <button type="button" class="tool" onclick="setAdminTerminalCommand('cd /home/hermes-duran/yoshi && code static/yoshi-app.js')">Open interface JS</button>
                    <button type="button" class="tool" onclick="setAdminTerminalCommand('cd /home/hermes-duran/yoshi && git status --short')">Check changes</button>
                    <button type="button" class="tool" onclick="setAdminTerminalCommand('cd /home/hermes-duran/yoshi && scripts/backup-yoshi-now')">Create backup</button>
                </div>
                ...
                <div class="admin-terminal-input-row">
                    <input id="adminTerminalInput" placeholder="Type a command, e.g. code static/yoshi-app.css" autocomplete="off">
            ...
            <section class="admin-card admin-card-wide agent-foundry-card">
                <div class="agent-foundry-heading">
                    <div>
                        ...
                        <h2>Build DiYoshi specialists</h2>
                    ...
                    <div class="agent-form-grid">
                        ...
                        <label class="settings-field agent-field-wide">
                            ...
                            <input id="agentPurpose" maxlength="240" required
                                   placeholder="Designs and checks DiYoshi interfaces">
                        </label>
                        <label class="settings-field">
                            <span>Brain</span>
        ...
        <div class="settings-stack">
            <details class="settings-group" open>
                ...
                <div class="settings-body">
                    ...
                    <label class="settings-toggle">
                        <span>Speak DiYoshi's replies</span>
                        <input type="checkbox" id="speakReplies">
                    </label>
                    <label class="settings-field" for="yoshiMode">
                        <span>DiYoshi mode</span>
                        <select id="yoshiMode"
                                onchange="applyYoshiMode()">
                    ...
                    <small class="permission-note">
                        Weather uses this saved city. DiYoshi does not read your
            ...
            <details class="settings-group">
                ...
                <div class="settings-body">
                    ...
                    <small class="permission-note">
                        DiYoshi cannot open a live camera by itself. A camera or
            ...
            <details class="settings-group"
                ...
                <div class="settings-body">
                    ...
                    <small class="permission-note">
                        ...
                        and an online connection. DiYoshi never grants Health access to another profile.
                    </small>
                </div>
            </details>
                ...
                <div class="settings-body profile-customizer">
                    ...
                    <label class="settings-field" for="profileAbout">
                        ...
                        <textarea id="profileAbout" maxlength="240" rows="3"
                                  placeholder="A few things DiYoshi should show on your profile."></textarea>
                    </label>
                    ...
                    <div id="adminCssControls" class="admin-css-controls" hidden>
                        ...
                        <label class="settings-field" for="profileCustomCss">
                            ...
                            <textarea id="profileCustomCss" rows="9" maxlength="12000"
                                      spellcheck="false"
                                      placeholder="/* Change DiYoshi directly, like a MySpace profile */"
                ...
                <div class="settings-body">
                    <label class="settings-field" for="voiceIdentity">
                        ...
                        <select id="voiceIdentity"
                            ...
                            <option value="diyoshi">🦖 DiYoshi — Friendly device voice</option>
                    ...
                    <div class="voice-settings-actions">
                        ...
                        <button type="button" class="tool"
                                onclick="stopYoshiVoice()">⏹ Stop voice</button>
                    </div>
            ...
            <details class="settings-group">
                ...
                <div class="settings-body">
                    <small style="color:var(--muted);">
                        ...
                        newest interface. Restart safely restarts DiYoshi's web
                        service. Your memories, profiles, and voice stay safe.
                    </small>
                    <div class="maintenance-actions">
                        <button type="button" class="tool"
                                onclick="refreshYoshiApp()">
                            ↻ Refresh app
                        </button>
                        <button type="button" class="tool"
                                onclick="clearYoshiCache()">
                            🧹 Clear cache
                        </button>
                        <button type="button"
                                class="tool maintenance-restart"
                                onclick="restartYoshiApp()">
                            🔄 Restart DiYoshi
                        </button>
                    </div>
                </div>
    ...
    ">
        ...
        <section id="spotifySetupSection">
            <p style="margin-top:0;">
                ...
                This is a one-time setup for this DiYoshi profile.
            </p>
            <ol class="spotify-setup-steps">
                ...
                <li>
                    Name it <strong>DiYoshi</strong>. If Spotify asks which
            ...
            <small style="display:block;margin-top:10px;color:var(--muted);">
                DiYoshi uses Spotify's PKCE sign-in. Your Spotify password and
        ...
        <section id="spotifyPlayerSection" hidden>
            <div class="spotify-player-card">
                <img id="spotifyAlbumArt" class="spotify-art"
                     src="/static/yoshi-icon.svg" alt="Current album art">
        ...
        <div class="watch-shell">
            ...
            <form class="watch-form" onsubmit="submitWatchText(event)">
                <input id="watchInput" maxlength="500"
                       placeholder="Ask DiYoshi…" aria-label="Watch message">
    ...
    <div id="searchPanel" class="search-hub-panel" style="display:none;">
        ...
        <p id="searchPrivacyNote" class="search-privacy-note">
            Results appear inside DiYoshi through Bing. Provider buttons open a new tab.
        </p>
    ...
    <div class="input-area" id="composer">
        <button id="thinkingBuddy" class="thinking-buddy" type="button"
                ...
                aria-label="DiYoshi buddy is ready" title="DiYoshi buddy">
            <img src="/static/diyoshi-avatar.png" alt="">
            <span>Ready</span>
        </button>
        ...
        <select id="brainMode" class="brain-mode"
                aria-label="DiYoshi brain" onchange="saveBrainMode()">
        ...
        <button id="stopThinkingButton"
                ...
                aria-label="Stop DiYoshi thinking"
                hidden>■ Stop</button>
    ...
    <nav class="mobile-nav" aria-label="DiYoshi app navigation">
        ...
        <button id="voiceDockButton" class="voice-dock" onclick="enableMicrophone()"
                aria-pressed="false" aria-label="Talk to DiYoshi">🎙️</button>
...
let activeProfile =
    localStorage.getItem("yoshi_active_profile") || "Hermes";
let activeChatSpace =
    localStorage.getItem("diyoshi_chat_space") === "mansion"
        ? "mansion"
        : "main";
let searchMode = "web";
...
function armProfileAutoLock() {
    ...
    if (
        ...
        sessionStorage.getItem("yoshi_signed_in_profile") !== record.name
    ) return;
...
function bringLockedProfileForward(profile, message) {
    ...
    if (!record || !record.locked || record.unlocked) return;
    sessionStorage.removeItem("yoshi_signed_in_profile");
...
function toggleSidebar() {
    ...
    if (window.matchMedia("(min-width: 900px)").matches) {
        ...
        localStorage.setItem("yoshi_sidebar_collapsed", String(collapsed));
...
const savedSidebarCollapsed = localStorage.getItem("yoshi_sidebar_collapsed");
...
function syncRoomHeader() {
    ...
    document.getElementById("appRoomTitle").textContent =
        mansion ? "🏰 Mansion DiYoshi" : "🦖 DiYoshi";
    input.placeholder = mansion
        ? "Build DiYoshi's final home…"
        : "Message DiYoshi…";
...
function renderMansionRoomBanner() {
    ...
    banner.innerHTML = `
        ...
        <h2>🏰 Mansion DiYoshi</h2>
        <p>Our dedicated workspace for building DiYoshi's final home.</p>
...
function mansionPrompt(area) {
    const prompts = {
        brain: "Help me plan the next dependable brain upgrade for DiYoshi.",
        desktop: "Help me build DiYoshi's final desktop application.",
        watch: "Help me build DiYoshi's Wear OS watch application.",
        home: "Help me plan DiYoshi's safe smart-home integration.",
        security: "Review Mansion DiYoshi's security and permission boundaries.",
        backup: "Review DiYoshi's backup, recovery, and rollback plan."
    };
...
async function openChatSpace(space) {
    ...
    localStorage.setItem("diyoshi_chat_space", activeChatSpace);
    conversationEpoch += 1;
...
function showProfileGreeting() {
    ...
    if (activeChatSpace === "mansion") {
        ...
        addMessage(
            ...
            ". This conversation stays separate from your regular DiYoshi chat.",
            "yoshi"
        );
        return;
    }
...
async function loadProfileHistory() {
    try {
        ...
        data.history.forEach(item => {
            ...
            if (item.role === "assistant") addMessage(item.content, "yoshi");
        });
        return true;
    } catch (error) {
        setProfileInteraction(true);
...
async function loadProfiles() {
    try {
        ...
        await refreshProfileCatalog();
        const signedIn =
            sessionStorage.getItem("yoshi_signed_in_profile") || "";
        ...
        localStorage.setItem("yoshi_active_profile", activeProfile);
        hideAccountChooser();
...
async function signIntoProfile(profileName) {
    ...
    localStorage.setItem("yoshi_active_profile", activeProfile);
    sessionStorage.setItem("yoshi_signed_in_profile", activeProfile);
    renderProfileSelectors();
    hideAccountChooser();
    closeSidebar();
    applyProfileExperience(record);
...
function chatPreferenceKey(name) {
    return "diyoshi-chat-" + String(activeProfile || "Hermes").toLowerCase()
        .replace(/[^a-z0-9]+/g, "-");
}
...
function setThinkingBuddy(state = "idle") {
    ...
    buddy.setAttribute(
        "aria-label",
        "DiYoshi buddy is " + (state === "thinking" ? "thinking" : "ready")
    );
    if (state === "happy") {
        window.setTimeout(() => {
...
async function sendText(text) {
    ...
    statusBox.textContent = "DiYoshi is thinking...";
    ...
    try {
        ...
        const replyElement = addMessage(reply, "yoshi");
        if (response.ok) {
            addLookupLinks(replyElement, text, data.sources);
        }

        if (response.ok && yoshiSettings.speak_replies) {
            const spokenReply = speakableReply(reply);
            if (spokenReply) speakYoshiReply(spokenReply);
        }
        ...
        addMessage(
            "I couldn't reach DiYoshi's local server.",
            "yoshi"
        );
...
async function checkHealth() {
    ...
    try {
        ...
        if (data.online === true) {
            status.textContent = "🟢 DiYoshi Online";

            if (splashText) {
                splashText.textContent = "DiYoshi is ready.";
            }
        ...
        status.textContent = "🟡 DiYoshi is waking up...";
        ...
        status.textContent = "🔴 DiYoshi Offline";

        if (splashText) {
            splashText.textContent = "Waiting for Yoshi...";
        }
    }
}
...
async function setYoshiMode(mode, preset) {
    try {
        ...
        yoshiSettings.mode = mode;
        yoshiSettings.voice_preset = preset;

        const modeSelect =
            document.getElementById("yoshiMode");
        ...
        statusBox.textContent =
            "Could not change DiYoshi mode.";
    }
}


async function openGamingDashboard() {
    await setYoshiMode("gaming", "dino");
...
const AI_TUTORIAL_LESSONS = [
    ...
    },
    {
        ...
        icon: "🔐",
        body: "Good personal AI should remember only what is useful and allowed. Sensitive features should be opt-in. In DiYoshi, profile locks, per-user permissions, and online/offline rules form the fence around private data.",
        tryPrompt: "Explain DiYoshi's privacy idea: opt-in permissions, profile locks, and online-only syncing, using a simple example."
    },
    {
        ...
        tryPrompt: "Give me a tiny beginner AI project I can build with DiYoshi, with exactly three steps."
    }
];
let aiTutorialIndex = 0;

function aiTutorialStorageKey() {
    return "diyoshi-ai-tutorial-" + String(activeProfile || "Hermes").toLowerCase();
}
...
async function loadDashboard() {
    ...
    try {
        ...
        box.textContent = "DiYoshi status unavailable.";
    }
}
...
async function createBackup() {
    statusBox.textContent = "Creating DiYoshi backup...";
...
function loadVoices() {
    ...
    const savedUri = localStorage.getItem("yoshi_voice_uri");
    let selectedIndex = 0;
    let selectedScore = -1;
    select.innerHTML = "";
    ...
    select.onchange = () => {
        ...
        if (voice) localStorage.setItem("yoshi_voice_uri", voice.voiceURI);
    };
}
...
function speakWithDeviceVoice(cleanText) {
    ...
    const voice = selectedYoshiVoice();
    ...
    speech.rate = Number(yoshiSettings.voice_rate ?? 1.0);
    speech.pitch = Number(yoshiSettings.voice_pitch ?? 0.95);
...
async function fetchCloneSpeech(text, engine) {
    const chosenEngine = engine || document.getElementById("voiceEngine")?.value || yoshiSettings.voice_engine || "edge";
...
function playCloneSpeech(audioBlob, requestToken) {
    return new Promise((resolve, reject) => {
        ...
        audio.playbackRate = Math.min(
            1.25,
            Math.max(0.75, Number(yoshiSettings.voice_rate ?? 1.0))
        );
...
async function speakYoshiReply(text) {
    const cleanText = cleanSpeechText(text).slice(0, 700);
    if (!cleanText) return;

    stopYoshiVoice();
    const requestToken = voiceRequestToken;
    const engineVal = document.getElementById("voiceEngine")?.value || yoshiSettings.voice_engine || "edge";
    const useServerVoice = ["edge", "piper", "clone"].includes(engineVal);
    ...
    if (useServerVoice) {
        try {
            ...
            if (requestToken !== voiceRequestToken) return;
            stopYoshiVoice();
            if (typeof statusBox !== "undefined" && statusBox) {
                statusBox.textContent =
                    "Using the device voice while server voice is unavailable.";
                setTimeout(() => {
...
function applyYoshiMode() {
    const mode =
        document.getElementById("yoshiMode").value;
...
function testVoice() {
    yoshiSettings.voice_identity =
        document.getElementById("voiceIdentity")?.value || "hermes";
    yoshiSettings.voice_engine =
        document.getElementById("voiceEngine")?.value || "edge";
    yoshiSettings.voice_rate = parseFloat(
        document.getElementById("voiceRate")?.value || "1.0"
    );
    yoshiSettings.voice_pitch = parseFloat(
        document.getElementById("voicePitch")?.value || "1.0"
    );

    if (typeof statusBox !== "undefined" && statusBox) {
        statusBox.textContent = "Synthesizing voice audio...";
    }

    speakYoshiReply(
        "Hello Hermes. This is DiYoshi, speaking with your chosen voice profile."
    ).finally(() => {
        setTimeout(() => {
            if (typeof statusBox !== "undefined" && statusBox && statusBox.textContent.includes("voice")) {
                statusBox.textContent = "";
            }
        }, 1500);
    });
...
function spotifySafeImage(value) {
    try {
        const url = new URL(value);
        return url.protocol === "https:" ? url.href : "/static/yoshi-icon.svg";
    } catch (error) {
        return "/static/yoshi-icon.svg";
    }
}
...
async function loadSpotifyNowPlaying() {
    ...
    try {
        ...
        art.onerror = () => { art.src = "/static/yoshi-icon.svg"; };
...
function talkToYoshiFromControl() {
    const panel = document.getElementById("adminPanel");
    if (panel) panel.style.display = "none";
    if (window.showChatHome) window.showChatHome();
...
async function refreshAdminControl() {
    ...
    try {
        ...
        summary.textContent = data.online
            ...
            : "DiYoshi is running with one or more degraded checks.";
...
async function startAdminTerminal() {
    ...
    try {
        const response = await fetch("/admin/terminal/start", {
            method: "POST",
            headers: {"Content-Type": "application/json"},
            body: JSON.stringify({profile: activeProfile, cwd: "/home/hermes-duran/yoshi"}),
        });
        const data = await response.json();
...
function squareAvatarBlob(file) {
    ...
    return new Promise((resolve, reject) => {
        ...
        image.onerror = () => {
            URL.revokeObjectURL(objectUrl);
            reject(new Error("DiYoshi could not open this photo."));
        };
        image.src = objectUrl;
    });
}
...
async function unlockProfileInside() {
    ...
    record.unlocked = true;
    if (target === activeProfile) {
        sessionStorage.setItem("yoshi_signed_in_profile", target);
        setProfileInteraction(true);
        hideAccountChooser();
        await loadProfileHistory();
    }
...
let yoshiRestarting = false;


function yoshiPause(milliseconds) {
    return new Promise(resolve => window.setTimeout(resolve, milliseconds));
}


function refreshYoshiApp() {
    statusBox.textContent = "Refreshing DiYoshi...";
    window.setTimeout(() => window.location.reload(), 120);
}


async function clearYoshiCache() {
    const confirmed = window.confirm(
        "Clear only DiYoshi's downloaded app cache? " +
        "Memories, profiles, settings, Spotify, and voice stay safe."
    );
    if (!confirmed) return;

    statusBox.textContent = "Clearing DiYoshi's app cache...";
    try {
        if ("caches" in window) {
        ...
        statusBox.textContent =
            "Cache cleared. Loading the newest DiYoshi interface...";
        await yoshiPause(350);
...
async function restartYoshiApp() {
    if (yoshiRestarting) return;

    const confirmed = window.confirm(
        "Restart DiYoshi's web service now? This takes about 5–10 seconds. " +
        "Memories, profiles, and the voice recording stay safe."
    );
    if (!confirmed) return;

    yoshiRestarting = true;
    closeSettings();
    statusBox.textContent = "Restarting DiYoshi...";
    ...
    try {
        const response = await fetch("/system/restart", {
            method: "POST",
            headers: { "X-Yoshi-Action": "restart" }
        });
        const data = await response.json();
        if (!response.ok) {
            statusBox.textContent =
                data.error || "DiYoshi could not restart.";
            yoshiRestarting = false;
            return;
        }
        previousInstance = data.instance || "";
    } catch (error) {
        statusBox.textContent =
            "Restart request sent. Waiting for DiYoshi...";
    }

    await yoshiPause(1000);
    for (let attempt = 0; attempt < 40; attempt += 1) {
        try {
            ...
            if (!response.ok) throw new Error("DiYoshi is restarting.");
            const data = await response.json();
            const newInstance =
                previousInstance && data.instance !== previousInstance;
            if (newInstance || (!previousInstance && sawOffline)) {
                statusBox.textContent = "DiYoshi is back online.";
                await yoshiPause(400);
        ...
        await yoshiPause(750);
    }

    statusBox.textContent =
        "DiYoshi is taking longer to restart. Try Refresh app in a moment.";
    yoshiRestarting = false;
}
...
async function saveSettings() {
    ...
    try {
        ...
        yoshiSettings = await response.json();
...
function openWatchMode() {
    ...
    setSideView("watch");
    updateWatchClock();
    if (!window.diyoshiWatchClock) {
        window.diyoshiWatchClock = window.setInterval(updateWatchClock, 30000);
    }
}
...
async function watchAsk(value) {
    ...
    if (!text) return;
    replyBox.textContent = "DiYoshi is thinking…";
    try {
        const response = await fetch("/chat", {
            method: "POST",
            headers: {"Content-Type": "application/json"},
        ...
        const data = await response.json().catch(() => ({}));
        if (!response.ok) {
            throw new Error(data.error || "DiYoshi could not answer.");
        }
        const reply = data.reply || "No response.";
        replyBox.textContent = reply;
        if (yoshiSettings.speak_replies) {
            const spoken = speakableReply(reply);
            if (spoken) speakYoshiReply(spoken);
        }
    } catch (error) {
...
function prepareSharedItem() {
    ...
    statusBox.textContent =
        "Shared with DiYoshi. Review it, then tap Send.";
    input.focus({ preventScroll: true });
...
async function newChat() {
    ...
    try {
        ...
        localStorage.removeItem("yoshi_chat_history");
        messages.innerHTML = "";
        if (activeChatSpace === "mansion") renderMansionRoomBanner();

        addMessage(
            activeChatSpace === "mansion"
                ? "A fresh Mansion DiYoshi build conversation is ready. " +
...
function setVoiceListening(active) {
    ...
    ["voiceButton", "voiceDockButton", "watchTalkButton"].forEach(id => {
        ...
        button.setAttribute(
            "aria-label",
            active ? "Stop listening" : "Talk to DiYoshi"
        );
    });
...
async function enableMicrophoneFor(destination = "chat") {
    ...
    if (!window.isSecureContext) {
        const message = "Open DiYoshi with the secure shared link to use the microphone.";
        if (microphoneStatus) microphoneStatus.textContent = message;
        addMessage(message, "yoshi");
        return;
    }

    if (sessionStorage.getItem("diyoshi_microphone_consent") !== "yes") {
        const approved = window.confirm(
            "DiYoshi will listen only after you tap Allow in the browser. " +
        ...
        sessionStorage.setItem("diyoshi_microphone_consent", "yes");
    }

    if (microphoneStatus) microphoneStatus.textContent =
        diyoshiNativeSpeechPlugin()
            ? "Waiting for Android's microphone permission…"
            : "Waiting for the browser's microphone prompt…";
    try {
        if (diyoshiNativeSpeechPlugin()) {
            if (microphoneStatus) microphoneStatus.textContent =
                "Android microphone ready. DiYoshi is listening.";
            return voice();
        }
        ...
        if (microphoneStatus) microphoneStatus.textContent =
            "Microphone ready. DiYoshi is listening.";
        voice();
    } catch (error) {
        sessionStorage.removeItem("diyoshi_microphone_consent");
        const message =
...
async function voice() {
    ...
    if (nativeSpeech) {
        ...
        try {
            const result = await nativeSpeech.startListening({
                language:
                    (yoshiSettings.voice_language || "en") + "-" +
                    (yoshiSettings.voice_region || "US")
            });
...
<script src="/static/yoshi-app.js?v=21"></script>

</body>
</html>
"""

history = yoshi_history.load_history("Hermes")
yoshi_settings.load_settings()
...
def profile_catalog():
    ...
    for member_id, name, role, _notes in yoshi_memory.get_family_members():
        clean = " ".join((name or "").split()).strip()
        if not clean or clean.casefold() in seen:
            continue
        seen.add(clean.casefold())
        flags = yoshi_roles.role_flags(clean, role)
        display_role = {
            "admin": "Administrator",
    ...
    for profile in profiles:
        ...
        version = yoshi_profile_avatar.avatar_version(profile["name"])
        profile["avatar_url"] = (
            f"/profile-avatar/{profile['id']}?v={version}"
            if version else None
        )
        profile["preferences"] = (
            yoshi_profile_preferences.get_preferences(profile["name"])
        )
    return profiles
...
def _profile_session_key(profile):
    return yoshi_profile_lock.profile_key(profile)


def _profile_session_unlocked(profile):
    if not yoshi_profile_lock.is_locked(profile):
        return True
...
def _remember_profile_unlock(profile):
    ...
    preferences = yoshi_profile_preferences.get_preferences(profile)
...
def _profile_locked_response(profile):
    auth_type = yoshi_profile_lock.status(profile)["auth_type"]
    return jsonify({
...
def enforce_profile_lock():
    ...
    except yoshi_profile_lock.ProfileLockError:
        return jsonify({
            "error": "Yoshi's profile security needs repair.",
...
def conversation_storage_profile(profile, space="main"):
    if space == "mansion":
        return f"{profile} Mansion DiYoshi"
    if space == "watch":
        return f"{profile} Watch DiYoshi"
    return profile
...
def chat_space_context(space):
    if space == "mansion":
        return (
            "You are in Mansion DiYoshi, a dedicated engineering room for "
            "building DiYoshi's dependable final home. Keep plans practical, "
            "family-first, private, testable, reversible, and organized."
        )
    if space == "watch":
        return (
            "You are answering from DiYoshi Watch mode. Be concise, useful, "
    ...
    with history_lock:
        ...
        if storage_profile not in profile_histories:
            profile_histories[storage_profile] = (
                yoshi_history.load_history(storage_profile)
            )
...
def append_conversation(
    ...
    with history_lock:
        ...
        if len(current) > 4:
            ...
            yoshi_summary.append_entries(older_messages, storage_profile)
            current = current[-4:]

        yoshi_history.save_history(current, storage_profile)
...
def reset_chat_history(profile="Hermes", space="main"):
    ...
    with history_lock:
        ...
        yoshi_history.clear_history(storage_profile)
        yoshi_summary.clear_summary(storage_profile)
...
def login_page():
    ...
    response = send_from_directory(
        "/home/hermes-duran/yoshi/static", "login.html"
    )
    response.headers["Cache-Control"] = "no-store"
    return response
...
def tech_preview():
    ...
    response = send_from_directory(
        "/home/hermes-duran/yoshi/static", "tech-shell.html"
    )
...
def home():
    ...
    if request.path == "/control":
        page = page.replace(
            ...
            1,
        ).replace("<title>DiYoshi</title>", "<title>DiYoshi Control</title>", 1).replace(
...
def app_version():
    watched = [
        ...
        os.path.join(app.root_path, "static", "yoshi-app.css"),
    ]
    version = max(
    ...
    if os.path.exists(manifest_path):
        try:
            ...
            releases = {}
    response = jsonify({"version": str(version), "releases": releases, "source": "DiYoshi main home"})
    response.headers["Cache-Control"] = "no-store"
    return response
...
def restart_service():
    if request.headers.get("X-Yoshi-Action") != "restart":
...
def health():
    model_online = False
    database_ok = False
    recall_ready = callable(getattr(yoshi_memory, "smart_recall", None))
    history_ok = yoshi_history.history_file_valid()
    settings_ok = yoshi_settings.settings_file_valid()
    profile_security_ok = yoshi_profile_lock.data_file_valid()
    profile_photos_ok = yoshi_profile_avatar.data_directory_valid()
    profile_preferences_ok = yoshi_profile_preferences.data_file_valid()
    news_service_ok = yoshi_news.service_ready()
    search_service_ok = yoshi_search.service_ready()
    voice_status = yoshi_voice_proxy.status()
    voice_service_ok = bool(voice_status.get("online"))
    try:
        yoshi_chat_attachments.ROOT.mkdir(parents=True, exist_ok=True)
        attachments_ok = os.access(yoshi_chat_attachments.ROOT, os.W_OK)
    except OSError:
        attachments_ok = False
    permission_queue_ok = callable(getattr(yoshi_permissions, "list_requests", None))

    try:
        model_online = bool(yoshi_memory.server_ready())
    except Exception:
        app.logger.exception("Yoshi model health check failed")

    try:
        database_ok = yoshi_memory.database_ready()
    except Exception:
        app.logger.exception("Yoshi database health check failed")
    ...
    response = jsonify({
        ...
        "brain_models": yoshi_memory.BRAIN_MODELS,
        "context_size": yoshi_memory.CONTEXT_SIZE
    })
...
def dashboard():
    ...
    battery = yoshi_memory.get_battery_status()
    status = yoshi_memory.get_device_status()
    ...
    weather = yoshi_memory.get_weather(weather_location)

    task_summary = yoshi_memory.get_task_summary()
...
def media_studio_status():
    ...
    if error:
        return error
    return jsonify(yoshi_media.status())
...
def media_studio_generate():
    ...
    try:
        if kind == "image":
            media = yoshi_media.generate_image(
                admin,
                data.get("prompt"),
    ...
    except yoshi_media.MediaError as exc:
...
def media_studio_video(job_id):
    ...
    if error:
        return error
    try:
        media = yoshi_media.poll_video(admin, job_id)
    except yoshi_media.MediaError as exc:
        return jsonify({"error": str(exc)}), 502
    if media.get("filename"):
        media["url"] = f"/generated-media/{media['filename']}?profile={admin}"
...
def generated_media(filename):
    ...
    if error:
        return error
    try:
        target = yoshi_media.media_path(admin, filename)
    except yoshi_media.MediaError as exc:
...
def _builder_project_directory(name):
    ...
    root = os.path.realpath(os.path.join(os.path.expanduser("~"), "yoshi", "projects"))
...
def builder_projects():
    ...
    if error:
        return error
    root = os.path.join(os.path.expanduser("~"), "yoshi", "projects")
...
def memory_center():
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if action == "add":
                message = yoshi_memory.add_memory(
                ...
                message = yoshi_memory.update_memory_record(
                    admin,
                    data.get("id"),
                    data.get("memory"),
                    category=data.get("category") or "General",
...
def equipment_health():
    ...
    if error:
        return error
    return jsonify({"equipment": yoshi_equipment_health.snapshot()})
...
def permission_requests_list():
    ...
    if error:
        return error
    return jsonify({"requests": yoshi_permissions.list_requests()})
...
def permission_requests_regression():
    ...
    if error:
        return error
    try:
        record = yoshi_permissions.create_regression_request(
            admin,
            data.get("summary") or "Run the full DiYoshi regression suite.",
        )
        return jsonify({"request": record}), 201
    except yoshi_permissions.PermissionRequestError as exc:
        return jsonify({"error": str(exc)}), 400
...
def permission_request_review(request_id):
    ...
    try:
        record = yoshi_permissions.review(
            request_id,
            admin,
            decision,
        )
...
def permission_request_rollback(request_id):
    ...
    if error:
        return error
    try:
        record = yoshi_permissions.rollback(request_id, admin)
        return jsonify({"request": record})
    except yoshi_permissions.PermissionRequestError as exc:
        return jsonify({"error": str(exc)}), 400
...
def family_profile_create():
    ...
    try:
        yoshi_profile_lock.validate_credential(credential, auth_type)
        yoshi_memory.add_family_member(name, "Family", "Invited by Hermes")
        profile = resolve_profile(name)
        if not profile:
            raise ValueError("The family profile could not be created.")
        yoshi_profile_lock.set_credential(profile, credential, auth_type)
        token, record = yoshi_invites.create(profile, admin)
    except ValueError as exc:
        return jsonify({"error": str(exc)}), 400
    except yoshi_profile_lock.ProfileLockError:
...
def family_invite_revoke(invite_id):
    ...
    if error:
        return error
    return jsonify({"invite": yoshi_invites.revoke(invite_id)})
...
def admin_profile_roles():
    ...
    if request.method == "POST":
        ...
        try:
            yoshi_roles.set_role(target, data.get("access_role"))
        except ValueError as exc:
            return jsonify({"error": str(exc)}), 400
    return jsonify({
        "profiles": [{
...
def family_profile_delete(profile_name):
    ...
    member = next(
        (
            item for item in yoshi_memory.get_family_members()
    ...
    if not member:
        return jsonify({"error": "That family account was not found."}), 404
    yoshi_invites.revoke_profile(target)
    yoshi_profile_lock.remove_pin(target)
    yoshi_profile_preferences.remove_preferences(target)
    yoshi_profile_avatar.remove_avatar(target)
    yoshi_roles.remove_role(target)
    yoshi_memory.remove_family_member(member[0])
    return jsonify({
        "ok": True,
        "removed": target,
...
def recovery_request():
    ...
    if profile and method in {"email", "sms"}:
        try:
            yoshi_recovery.request_code(profile, method)
        except Exception:
...
def recovery_verify():
    ...
    profile = resolve_profile(data.get("username"))
    token = (
        yoshi_recovery.verify_code(profile, data.get("code"))
...
def recovery_reset():
    ...
    if not profile or not yoshi_recovery.consume_reset_token(
        profile, data.get("reset_token")
    ):
        return jsonify({
            "error": "The recovery session is incorrect or expired."
        }), 400
    result = yoshi_profile_lock.set_credential(
...
def family_login():
    ...
    if not credential or not str(credential).strip():
        return jsonify({"error": "PIN or password is required to unlock."}), 400

    locked = bool(yoshi_profile_lock.is_locked(profile))
    if locked:
        valid = yoshi_profile_lock.verify_credential(profile, credential)
    else:
        auth_type = "pin" if str(credential).strip().isdigit() else "password"
        try:
            yoshi_profile_lock.set_credential(profile, str(credential).strip(), auth_type)
            valid = True
...
def family_invite_claim():
    ...
    data = data if isinstance(data, dict) else {}
    try:
        profile = yoshi_invites.claim(data.get("token"))
    except ValueError as error:
...
def recovery_profile_settings():
    ...
    if request.method == "GET":
        return jsonify({
            "contact": yoshi_recovery.get_profile_contact(profile),
            "delivery": yoshi_recovery.delivery_status(),
        })
    try:
        contact = yoshi_recovery.save_profile_contact(
            profile,
        ...
        return jsonify({"error": str(error)}), 400
    except yoshi_recovery.RecoveryError:
...
def recovery_delivery_settings():
    ...
    data = data if isinstance(data, dict) else {}
    if request.method == "GET":
        return jsonify({"delivery": yoshi_recovery.delivery_status()})
    try:
        delivery = yoshi_recovery.save_delivery_settings(data)
...
def profiles_get():
    try:
        ...
        for item in profile_catalog():
            profile = dict(item)
            security = yoshi_profile_lock.status(item["name"])
        ...
        yoshi_profile_lock.ProfileLockError,
        yoshi_profile_preferences.ProfilePreferencesError,
    ):
        return jsonify({
            "error": "DiYoshi's profile data needs repair.",
...
def news_get():
    ...
    try:
        return jsonify(yoshi_news.get_headlines(topic, force=force))
    except ValueError as error:
        return jsonify({"error": str(error)}), 400
...
def web_search_get():
    try:
        return jsonify(yoshi_search.web_search(request.args.get("q", "")))
    except ValueError as error:
        return jsonify({"error": str(error)}), 400
...
def device_search_get():
    ...
    try:
        return jsonify(yoshi_search.device_search(request.args.get("q", "")))
    except ValueError as error:
        return jsonify({"error": str(error)}), 400
...
def profile_preferences_get():
    ...
    except yoshi_profile_preferences.ProfilePreferencesError:
...
def profile_preferences_save():
    ...
    try:
        preferences = yoshi_profile_preferences.save_preferences(
            profile,
            data.get("preferences", {}),
        )
        if yoshi_profile_lock.is_locked(profile):
            _remember_profile_unlock(profile)
...
def profile_avatar_get(profile_id):
    ...
    path = yoshi_profile_avatar.avatar_path(profile["name"])
    if path is None:
...
def _avatar_request_profile(value):
    ...
    try:
        ...
        return None, (
            jsonify({"error": "Yoshi's profile security needs repair."}),
            503,
        )
    return profile, None
...
def profile_avatar_upload():
    ...
    try:
        data = upload.stream.read(yoshi_profile_avatar.MAX_AVATAR_BYTES + 1)
        yoshi_profile_avatar.save_avatar(profile, data)
        item = next(
    ...
    except (yoshi_profile_avatar.ProfileAvatarError, OSError) as error:
        return jsonify({"error": str(error)}), 400
...
def profile_avatar_remove():
    ...
    if error:
        return error
    try:
        removed = yoshi_profile_avatar.remove_avatar(profile)
        return jsonify({
            "profile": profile,
            "removed": removed,
            "avatar_url": None,
...
def profile_lock_unlock():
    ...
    try:
        ...
        security = yoshi_profile_lock.status(profile)
        ...
        if not yoshi_profile_lock.verify_credential(profile, credential):
...
def profile_lock_configure():
    ...
    try:
        already_locked = yoshi_profile_lock.is_locked(profile)
        if already_locked and not _profile_session_unlocked(profile):
            ...
            if not yoshi_profile_lock.verify_credential(profile, current):
                return _credential_failure_response(profile)
            _clear_profile_failures(profile)
...
def profile_lock_remove():
    ...
    try:
        ...
        yoshi_profile_lock.remove_pin(profile)
        _forget_profile_unlock(profile)
        return jsonify({
            "profile": profile,
            "locked": False,
...
def health_sync_status():
    ...
    result = yoshi_health_access.authorize(
        profile, online=online, unlocked=_profile_session_unlocked(profile)
    )
    result["available_permissions"] = list(yoshi_health_access.PERMISSIONS)
...
def health_sync_configure():
    ...
    if profile is None:
        return jsonify({"error": "Unknown profile."}), 400
    try:
        policy = yoshi_health_access.set_policy(
            profile,
...
def health_sync_authorize():
    ...
    yoshi_health_access.record_sync_access(profile, categories, result["allowed"])
...
def settings_get():
    return jsonify(yoshi_settings.load_settings())


@app.route("/voice-status", methods=["GET"])
def voice_status():
    return jsonify(yoshi_voice_proxy.status())
...
def _spotify_callback_uri():
    configured = os.environ.get("YOSHI_SPOTIFY_REDIRECT_URI", "").strip()
    if configured:
        return configured
...
def spotify_status():
    ...
    payload = yoshi_spotify.status(profile)
    payload.update({
        "callback_uri": callback_uri,
        "callback_supported": yoshi_spotify.redirect_uri_supported(callback_uri),
...
def spotify_config():
    ...
    try:
        yoshi_spotify.save_client_id(data.get("client_id"))
...
def spotify_connect():
    ...
    try:
        profile = _spotify_request_profile(data)
        authorize_url = yoshi_spotify.begin_authorization(
            profile,
...
def spotify_callback():
    ...
    try:
        yoshi_spotify.complete_authorization(
            request.args.get("state"),
            request.args.get("code"),
        )
    except yoshi_spotify.SpotifyError:
...
def spotify_disconnect():
    ...
    yoshi_spotify.disconnect(profile)
...
def spotify_search():
    try:
        profile = _spotify_request_profile()
        results = yoshi_spotify.search_tracks(
            profile,
            request.args.get("q", ""),
            limit=5,
        )
...
def spotify_playlists():
    try:
        profile = _spotify_request_profile()
        results = yoshi_spotify.playlists(profile, limit=10)
    except ValueError as error:
        return jsonify({"error": str(error)}), 400
    except yoshi_spotify.SpotifyError as error:
        return _spotify_error(error)
...
def spotify_now_playing():
    try:
        profile = _spotify_request_profile()
        result = yoshi_spotify.now_playing(profile)
    except ValueError as error:
        return jsonify({"error": str(error)}), 400
    except yoshi_spotify.SpotifyError as error:
        return _spotify_error(error)
...
def speak():
    ...
    try:
        audio = yoshi_voice_proxy.synthesize(text, engine=engine, voice=voice)
    except ValueError:
        return jsonify({"error": "Text is required."}), 400
    except yoshi_voice_proxy.VoiceUnavailable as error:
    ...
    return Response(
        ...
        headers={
            ...
            "Content-Disposition": "inline; filename=yoshi-voice.wav",
            "X-Yoshi-Voice": "Hermes",
        },
    )
...
def settings_post():
    ...
    if "voice_preset" in data:
        ...
        if preset in ("custom", "calm", "tech", "dino", "gentle", "friendly", "professional"):
    ...
    yoshi_settings.save_settings(settings)
...
def tasks_done(task_id):
    reply = yoshi_memory.complete_task(task_id)
...
def family_create():
    ...
    reply = yoshi_memory.add_family_member(
        name,
        role or "Family",
        notes
    )
...
def family_update(member_id):
    ...
    reply = yoshi_memory.update_family_member(
        member_id,
        data.get("name"),
        data.get("role"),
        data.get("notes")
...
def family_delete(member_id):
    reply = yoshi_memory.remove_family_member(member_id)
    return jsonify({"reply": reply})
...
def shopping_update(item_id):
    ...
    reply = yoshi_memory.set_shopping_bought(
        item_id,
        bought
    )
...
def shopping_delete(item_id):
    reply = yoshi_memory.delete_shopping_item(item_id)
    return jsonify({"reply": reply})
...
def _enabled_hermes_agent():
    agent = yoshi_agents.get_agent_by_name("Hermes")
...
def hermes_ai_history():
    ...
    with history_lock:
        if request.method == "DELETE":
            yoshi_history.clear_history(conversation)
            return jsonify({"cleared": True})
        messages = yoshi_history.load_history(conversation)
    return jsonify({
        "messages": messages,
...
def hermes_ai_chat():
    ...
    try:
        attachment_context, attachment_items = yoshi_chat_attachments.prompt_context(
            admin, attachment_ids
        )
        image_payloads = yoshi_chat_attachments.image_payloads(
            admin, attachment_ids
        )
    except yoshi_chat_attachments.AttachmentError:
    ...
    conversation = "Agent " + agent["id"]
    with history_lock:
        recent = yoshi_history.load_history(conversation)
    ...
    diagnostic_intent = bool(re.search(
        r"\b(diagnos\w*|troubleshoot\w*|run\s+(?:python\s+)?checks?|inspect\s+diyoshi|check\s+diyoshi)\b",
        message,
        re.IGNORECASE,
    ))
    if diagnostic_intent and not image_payloads:
        try:
            existing = next(
                (
                    item for item in yoshi_permissions.list_requests(100)
                    if item.get("action") == "run_regression"
                    and item.get("status") == "pending"
                ),
                None,
            )
            diagnostic = existing or yoshi_permissions.create_regression_request(
                admin,
                "Hermes diagnosis: run DiYoshi Python compile and regression checks",
            )
            request_label = str(diagnostic.get("id") or "")[:8]
            state = "already waiting" if existing else "now waiting"
            reply = (
                "Yes. I can diagnose DiYoshi and inspect its project code. "
        ...
        try:
            reply = yoshi_memory.ask_yoshi(
                ...
                system_context=(
                    ...
                    "You can inspect the current DiYoshi source files with read-only project tools. "
                    ...
                    "Never say you lack access to DiYoshi code or internal systems; use your "
                    ...
                    + yoshi_agents.agent_system_context(agent)
                ),
    ...
    with history_lock:
        yoshi_history.save_history(updated, conversation)
    return jsonify({
        "reply": reply,
        "agent": {"id": agent["id"], "name": agent["name"]},
...
def admin_agents_collection():
    ...
    if request.method == "GET":
        return jsonify({
            "agents": yoshi_agents.list_agents(),
            "model_modes": yoshi_agents.MODEL_MODES,
            "memory_scopes": yoshi_agents.MEMORY_SCOPES,
            "capabilities": yoshi_agents.CAPABILITIES,
            "approval_required": True,
        })

    try:
        agent = yoshi_agents.save_agent(data)
    except ValueError as error:
        return jsonify({"error": str(error)}), 400
    except OSError:
        app.logger.exception("Could not save DiYoshi agent")
...
def admin_agent_delete(agent_id):
    ...
    if error:
        return error
    if not yoshi_agents.delete_agent(agent_id):
...
def admin_agent_test(agent_id):
    ...
    agent = yoshi_agents.get_agent(agent_id)
    if not agent:
        return jsonify({"error": "That AI was not found."}), 404
    ...
    model_name = yoshi_memory.choose_brain_model(
    ...
    try:
        ...
        app.logger.exception("DiYoshi agent test failed")
...
def admin_migration_create():
    ...
    script = os.path.join(app.root_path, "scripts", "backup-yoshi-now")
    try:
        ...
        app.logger.exception("DiYoshi migration package failed")
...
def backup_create():
    reply = yoshi_memory.create_backup()
    return jsonify({"reply": reply})
...
def reminders_done(reminder_id):
    reply = yoshi_memory.complete_reminder(reminder_id)
    return jsonify({"reply": reply})
...
def routine_done_today_route(routine_id):
    reply = yoshi_memory.complete_routine_for_today(routine_id)
    return jsonify({"reply": reply})
...
def gaming_dashboard():
    ...
    battery = yoshi_memory.get_battery_status()
    status = yoshi_memory.get_device_status()
    ai_online = yoshi_memory.server_ready()
...
def _messaging_profile_request(payload=None):
    ...
    yoshi_messaging.touch_presence(profile)
    return profile, None
...
def messaging_people():
    ...
    return jsonify({
        "profile": profile,
        "people": yoshi_messaging.people(profile_catalog(), profile),
    })
...
def messaging_conversations_get():
    ...
    return jsonify({
        "profile": profile,
        "conversations": yoshi_messaging.list_conversations(profile),
    })
...
def messaging_conversations_create():
    ...
    try:
        conversation_id = yoshi_messaging.create_conversation(
            profile,
            resolved_members,
...
def messaging_messages_get(conversation_id):
    ...
    if error:
        return error
    try:
        messages = yoshi_messaging.get_messages(
            conversation_id,
            profile,
...
def messaging_messages_post(conversation_id):
    ...
    if error:
        return error
    try:
        message = yoshi_messaging.send_message(
...
def messaging_messages_read(conversation_id):
    ...
    if error:
        return error
    try:
        result = yoshi_messaging.mark_read(
...
def messaging_typing_post():
    ...
    try:
        users = yoshi_messaging.set_typing(
            conversation_id,
            profile,
...
def messaging_typing_get(conversation_id):
    ...
    if error:
        return error
    try:
        users = yoshi_messaging.get_typing_users(conversation_id, profile)
    except PermissionError:
        return jsonify({"error": "Conversation access denied."}), 403
    return jsonify({
        "conversation_id": conversation_id,
...
def chat_attachment_upload():
    ...
    try:
        data = upload.stream.read(yoshi_chat_attachments.MAX_BYTES + 1)
        item = yoshi_chat_attachments.save_attachment(
    ...
    except yoshi_chat_attachments.AttachmentError as error:
        return jsonify({"error": str(error)}), 400
    except OSError:
...
def chat_attachment_get(attachment_id):
    ...
    if profile is None:
        return jsonify({"error": "Choose a valid profile first."}), 400
    try:
        metadata, path = yoshi_chat_attachments.get_attachment(
...
def chat():
    try:
        ...
        app.logger.exception("Unhandled exception in Yoshi chat")
        return jsonify({
            "reply": "Yoshi ran into a problem while answering. Please try again.",
            "error": "chat_failed"
        }), 500
...
def _chat_impl():
    ...
    try:
        attachment_context, attachment_items = (
            yoshi_chat_attachments.prompt_context(
    ...
    if space is None:
        return jsonify({
            "reply": "Choose a valid DiYoshi room.",
            "error": "invalid_space"
        }), 400
    ...
    if text.startswith("/note "):
        return jsonify({
            "reply": yoshi_memory.add_note(text[len("/note "):])
        })
    ...
    if text.startswith("/task "):
        return jsonify({
            "reply": yoshi_memory.add_task(text[len("/task "):])
        })
    ...
    if text.startswith("/done "):
        return jsonify({
            "reply": yoshi_memory.complete_task(text[len("/done "):])
        })

    if text.startswith("/remember-shared "):
        reply = yoshi_memory.add_memory(
    ...
    if text == "/memories":
        memories = yoshi_memory.get_memories(profile=profile)
    ...
    if text.startswith("/memories "):
        ...
        if memory_filter.lower() == "inactive":
            memories = [
                row for row in yoshi_memory.get_memories(
            ...
            memories = yoshi_memory.get_memories(
        ...
        reply = yoshi_memory.format_memory_rows(memories)
        return jsonify({"reply": reply})
    ...
    if text.startswith("/find-memory "):
        ...
        memories = yoshi_memory.smart_recall(
            query,
            limit=10,
            profile=profile
        )
    ...
    if text.startswith("/restore-memory "):
        ...
        reply = yoshi_memory.restore_memory(
            memory_id,
            profile=profile
        )
    ...
    if text == "/battery":
        return jsonify({
            "reply": yoshi_memory.get_battery_status()
        })

    if text == "/status":
        return jsonify({
            "reply": yoshi_memory.get_device_status()
        })

    if text == "/weather":
        return jsonify({
            "reply": yoshi_memory.get_weather(
    ...
    if not incognito and settings.get("auto_memory", True):
        saved = yoshi_memory.auto_remember(text, profile=profile)
    else:
        saved = False
    ...
    try:
        ...
        app.logger.exception("Yoshi could not complete a chat request")
        return jsonify({
            "reply": "Yoshi's local AI could not answer right now.",
...
def stop_owned_model(process):
    ...
    try:
        with open(yoshi_memory.MODEL_PID_PATH, encoding="utf-8") as handle:
            recorded_pid = int(handle.read().strip())
        if recorded_pid == process.pid:
            os.unlink(yoshi_memory.MODEL_PID_PATH)
    except (FileNotFoundError, OSError, TypeError, ValueError):
        pass


if __name__ == "__main__":
    yoshi_memory.connect_db().close()
    owned_model = None
    ...
    try:
        owned_model = yoshi_memory.start_server()
        app.run(
            host="127.0.0.1",
            port=5000,
            debug=False

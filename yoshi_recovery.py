...
def request_code(profile, method, provided_destination=None):
    ...
    with _LOCK:
        ...
        if provided_destination and str(provided_destination).strip():
            clean_prov = str(provided_destination).strip()
            if method == "email":
                if not EMAIL_PATTERN.fullmatch(clean_prov.lower()):
                    raise ValueError("Enter a valid email address.")
                clean_prov = clean_prov.lower()
            else:
                clean_prov = re.sub(r"[\s().-]", "", clean_prov)
                if re.fullmatch(r"\d{10}", clean_prov):
                    clean_prov = "+1" + clean_prov
                elif not PHONE_PATTERN.fullmatch(clean_prov):
                    raise ValueError("Phone number must include country code, e.g. +15551234567.")

            if destination:
                if destination.casefold() != clean_prov.casefold() and destination.replace("+", "") != clean_prov.replace("+", ""):
                    raise RecoveryError("Provided " + ("email" if method == "email" else "phone number") + " does not match the registered recovery contact for " + str(profile) + ".")
            else:
                if method == "email":
                    save_profile_contact(profile, email=clean_prov)
                else:
                    save_profile_contact(profile, phone=clean_prov)
                destination = clean_prov

        if not destination:
            raise RecoveryError("No recovery " + ("email" if method == "email" else "phone number") + " is registered for this profile. Please provide one.")
    ...
    # Log to local secure recovery log for workstation backup
    try:
        log_file = BASE_DIR / "recovery_codes.log"
        log_file.parent.mkdir(parents=True, exist_ok=True)
        with open(log_file, "a", encoding="utf-8") as f:
            f.write(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Profile: {profile} | Method: {method} | Destination: {destination} | Code: {code}\n")
    ...
    return {
        ...
        "message": f"A 6-digit verification code has been sent to your {method.upper()} ({_mask_email(destination) if method == 'email' else _mask_phone(destination)}).",
        "dev_code": code if not sent_externally else None,
    }
